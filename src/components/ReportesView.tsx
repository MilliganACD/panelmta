...
export default function ReportesView({ ventas }: ReportesViewProps) {
  ...
  const totalCobrarInActiveSet = useMemo(() => {
    return completedVentas.reduce((sum, v) => sum + v.pendingAmount, 0);
  }, [completedVentas]);

  const handleSimulatePDF = () => {
    alert('📥 [PDF GENERATOR] Compilando reporte integral corporativo...\nSe han consolidado las métricas de venta diaria e histórico de deudas.\nArchivo "SPORTS_POS_REPORTE_24MAYO.pdf" guardado en descargas.');
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white border border-slate-200 p-5 rounded-xl flex flex-col justify-between hover:shadow-sm">
          ...
          <div className="mt-4">
            <span className="text-2xl font-black text-slate-800 font-mono">S/ {(4280.50 + totalSalesVal - 74.50).toFixed(2)}</span>
        ...
        <div className="bg-white border border-slate-200 p-5 rounded-xl flex flex-col justify-between hover:shadow-sm">
          ...
          <div className="mt-4">
            <span className="text-2xl font-black text-rose-600 font-mono">S/ {(120.00 + totalCobrarInActiveSet).toFixed(2)}</span>
            <p className="text-rose-500 text-[11px] font-bold mt-1">Por regularizar en cuentas</p>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 p-5 rounded-xl flex flex-col justify-between hover:shadow-sm">
          ...
          <div className="mt-4">
            <span className="text-2xl font-black text-slate-800">8 Clientes</span>
        ...
        <div className="bg-white border border-slate-200 p-5 rounded-xl flex flex-col justify-between hover:shadow-sm">
          ...
          <div className="mt-4">
            <span className="text-2xl font-black text-slate-800 font-mono">S/ {ticketPromedio > 0 ? ticketPromedio.toFixed(2) : '12.50'}</span>
            <p className="text-rose-500 text-[11px] font-bold mt-1 flex items-center gap-1">
              <TrendingDown className="w-3 h-3" /> -2.1% en compras rápidas
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 bg-white border border-slate-200 p-5 rounded-xl space-y-4">
          ...
          <div className="h-64 flex items-end justify-between gap-3 pt-6 px-4">
            {[
              { day: 'Lun', val: 45, price: 'S/ 420' },
              { day: 'Mar', val: 65, price: 'S/ 610' },
              { day: 'Mie', val: 55, price: 'S/ 520' },
              { day: 'Jue', val: 80, price: 'S/ 780' },
              { day: 'Vie', val: 100, price: 'S/ 940', highlighted: true },
              { day: 'Sab', val: 35, price: 'S/ 310' },
              { day: 'Dom', val: 25, price: 'S/ 180' }
            ].map((col, idx) => {
              return (
                  ...
                  <div 
                    ...
                    className={`w-full rounded-t-lg transition-all duration-300 transform group-hover:scale-x-105 ${
                      col.highlighted 
                        ...
                        : 'bg-slate-200 group-hover:bg-teal-100'
                    }`}
                  ></div>

                  <span className={`text-[10px] uppercase font-mono font-black mt-3 ${col.highlighted ? 'text-teal-600' : 'text-slate-400'}`}>
                    {col.day}
                  </span>
                </div>
              );
          ...
          <div className="space-y-4 mt-6 flex-1">
            ...
            <div className="space-y-1">
              <div className="flex justify-between items-center text-xs font-semibold">
                <span className="text-slate-700">💧 Agua Mineral / San Mateo</span>
                <span className="text-teal-600 font-bold">142 unid.</span>
              </div>
            ...
            <div className="space-y-1">
              <div className="flex justify-between items-center text-xs font-semibold">
                <span className="text-slate-700">🥤 Coca Cola</span>
                <span className="text-teal-600 font-bold">98 unid.</span>
              </div>
              <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
                <div className="bg-teal-600 h-full rounded-full" style={{ width: '65%' }}></div>
              </div>
            </div>
            ...
            <div className="space-y-1">
              <div className="flex justify-between items-center text-xs font-semibold">
                <span className="text-slate-700">🍪 Oreo</span>
                <span className="text-teal-600 font-bold">76 unid.</span>
              </div>
              <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
            ...
            <div className="space-y-1">
              <div className="flex justify-between items-center text-xs font-semibold">
                <span className="text-slate-700">🍫 Sublime</span>
                <span className="text-teal-600 font-bold">62 unid.</span>
              </div>
              <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
            ...
            <div className="space-y-1">
              <div className="flex justify-between items-center text-xs font-semibold">
                <span className="text-slate-700">🍿 Chizitos</span>
                <span className="text-teal-600 font-bold">54 unid.</span>
              </div>
              <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
                <div className="bg-teal-600 h-full rounded-full" style={{ width: '36%' }}></div>
              </div>
            </div>
